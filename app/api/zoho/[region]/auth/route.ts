...
export async function GET(req: NextRequest, { params }: { params: { region: 'IN'|'US'|'EU'|'UK' } }){
  ...
  const url = `${accounts}/oauth/v2/auth?response_type=code&client_id=${id}&scope=${scope}&redirect_uri=${encodeURIComponent(redirect_uri)}&access_type=offline&prompt=consent`;
